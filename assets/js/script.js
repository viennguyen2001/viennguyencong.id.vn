...
(function ($) {
  ...
  $("#legacyContactForm").on("submit", function (e) {
    ...
    if (
      ...
      try {
        var storageKey = "nino-dashboard-content";
        var dashboardData = JSON.parse(localStorage.getItem(storageKey) || "{}");
        var messageItem = {
          id: Date.now(),
          title: data.subject,
          owner: data.name,
        ...
        var contactMessages = dashboardData.contact || [];
        var isDuplicate = contactMessages.some(function (item) {
          return (
            item.email === messageItem.email &&
            item.title === messageItem.title &&
            item.summary === messageItem.summary
          );
        });

        if (!isDuplicate) {
          dashboardData.contact = [messageItem].concat(contactMessages);
          localStorage.setItem(storageKey, JSON.stringify(dashboardData));
        }
      } catch (error) {}

      $("#contactForm .input-success")
        .text("Message saved. I will get back to you soon!")
        .stop(true, true)
        .fadeIn(300);
      $("#contactForm .input-error").hide();
      ...
      $("#contactForm .input-error").stop(true, true).fadeIn(300);
      $("#contactForm .input-success").hide();
      $("#msgSubmit")
        .removeClass("hidden text-success")
        .addClass("h4 text-left text-danger")
        .text("Please fill in all contact fields correctly.");
    }
